...
| **Write-combined framebuffer** | The blob framebuffer is allocated write-combined (real-hardware semantics): CPU writes bypass the cache, so with KVM honoring the guest PAT the host sees WC pages and a dmabuf import has no CPU cache to flush — eliminates the per-frame `wbinvd`/IPI storms on the host (measured: ~20% of a host core per active VM). | `viogpu-blob` | `-accel kvm,honor-guest-pat=on` |
